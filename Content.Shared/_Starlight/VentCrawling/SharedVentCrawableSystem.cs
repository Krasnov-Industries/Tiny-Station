...
private static readonly Dictionary<MoveButtons, Direction> ButtonToDirection = new()
{
...
private void OnMoveInput(EntityUid uid, VentCrawlerHolderComponent holder, ref MoveInputEvent args)
{
    ...
    if (TryComp<InputMoverComponent>(uid, out var mover))
    {
        ...
        foreach (var (button, dir) in ButtonToDirection)
        {
            if ((mover.HeldMoveButtons & button) != 0)
            {
