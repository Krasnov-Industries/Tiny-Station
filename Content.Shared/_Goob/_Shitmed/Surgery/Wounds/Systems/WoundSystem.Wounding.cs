...
using System.Linq;
using Content.Shared._Goob._Shitmed.CCVar;
using Content.Shared._Goob._Shitmed.DoAfter;
using Content.Shared._Goob._Shitmed.Medical.Surgery.Pain.Components;
using Content.Shared._Goob._Shitmed.Medical.Surgery.Traumas;
using Content.Shared._Goob._Shitmed.Medical.Surgery.Traumas.Components;
...
using Content.Shared._Goob._Shitmed.Targeting.Events;
...
namespace Content.Shared._Goob._Shitmed.Medical.Surgery.Wounds.Systems;
...
public sealed partial class WoundSystem
{
    ...
    private void OnDamageChanged(EntityUid uid, WoundableComponent component, ref DamageChangedEvent args)
    {
        ...
        if (args.DamageDelta == null // Goobstation
        ...
        foreach (var (damageType, damageValue) in args.DamageDelta.DamageDict)
        {
            ...
            if (damageValue < 0)
            {
                TryHealWoundsOnWoundable(uid, -damageValue, damageType, out var healed, component, ignoreBlockers: false);
            }
            else
            {
                ...
                TryInduceWound(uid,
                    damageType,
                    damageValue *
                    args.DamageDelta.WoundSeverityMultipliers.GetValueOrDefault(damageType, 1),
                    out _,
                    component);
            }
        }
