...
4. MUST: if the entity is unique to the fork and especially if it is a forked copy of the vanilla entity, add the fork prefix (`TinystationXxx` for new content; keep `GoobXxx`/`EEXxx` when editing inherited entities).
...
5. Are you creating a fork-only or fork copy of vanilla?
   Add a fork prefix to the ID (`Tinystation*` for new content; preserve `Goob*`/`EE*` only when extending inherited entities).
