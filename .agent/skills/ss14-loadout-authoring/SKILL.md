...
3. This skill does not replace `ss14-upstream-maintenance`: when copying vanilla prototypes to the fork, still keep the changes isolated inside `_Tinystation/`. Inherited folders `_Goobstation/` and `_EE/` may already contain loadout content ported earlier — see [[ss14-upstream-maintenance]] for the policy on inherited folders.
...
1. `loadout`: `TinystationLoadout` + `ItemPrototypeId`.
2. `loadoutGroup`: `TinystationLoadoutGroup` + slot name or generic group name.
3. `roleLoadout`: `Job<RoleId>`.
4. If the same `ItemPrototypeId` is needed in two different contexts and without the suffix there will be a collision, add a short context tail: `TinystationLoadoutFlashlightLanternCargo`, `TinystationLoadoutFlashlightLanternEngineering`.
5. Do not leave new local IDs without a fork prefix.
6. When editing loadout content that already lives in `_Goobstation/` or `_EE/`, keep its existing prefix (`GoobLoadout*`, `EELoadout*` or whatever the inherited code uses) — do not rename inherited IDs just to unify naming; that breaks save compatibility.
...
2. The local path must mirror the original path inside `_Tinystation/`.
3. Don't put copies in technical dumps like `Imports/...` if you can put them in a normal mirror path.
...
1. Find all `_Tinystation` (or inherited `_Goobstation`/`_EE`) job prototypes that the task concerns.
...
1. Make sure that all `_Tinystation roleLoadout -> loadoutGroup -> loadout` links are local to the fork (including inherited `_Goobstation`/`_EE` content where applicable).
...
3. Give `loadout` ID in the format `TinystationLoadout<ItemPrototypeId>`.
4. Give `loadoutGroup` ID in the format `TinystationLoadoutGroup<SlotOrConcept>`, and not according to the historical copy source.
...
1. Leave a `_Tinystation` role to refer to a vanilla or upstream-fork `loadoutGroup` (vanilla, third-party, or untouched inherited group) — copy it locally first.
...
```yaml
- type: loadout
  id: TinystationLoadoutKeyCardMedicalSpecialist
  equipment:
    id: KeyCardMedicalSpecialist

- type: loadoutGroup
  id: TinystationLoadoutGroupIdKeyCardMedicalSpecialist
  name: loadout-group-id
  minLimit: 1
  loadouts:
  - TinystationLoadoutKeyCardMedicalSpecialist

- type: roleLoadout
  id: JobMedicalSpecialist
  groups:
  - TinystationLoadoutGroupIdKeyCardMedicalSpecialist
```
...
```yaml
- type: loadoutGroup
  id: TinystationLoadoutGroupMedicalShoes
  name: loadout-group-shoes
  loadouts:
  - TinystationLoadoutClothingShoesColorWhite
  - TinystationLoadoutClothingShoesBootsWinterMed
...
```yaml
- type: loadoutGroup
  id: TinystationLoadoutGroupSecurityBackpack
  name: loadout-group-backpack
  loadouts:
  - TinystationLoadoutClothingBackpackSecurity
  - TinystationLoadoutClothingBackpackSatchelSecurity
  - TinystationLoadoutClothingBackpackDuffelSecurity
```
...
1. All new IDs correspond to the scheme `TinystationLoadout*` / `TinystationLoadoutGroup*` / `Job<RoleId>`.
