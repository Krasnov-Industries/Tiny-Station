...
### 9) Pattern: world-bounds into local tree coordinates (e.g. for FOV/overlay queries)

```csharp
// For each component-tree, first translate worldBounds into local tree coordinates.
...
### 10) Pattern: direction-only transform (e.g. for hitscan ricochet)

When transforming a direction vector with an inverse world matrix, zero the translation row first — otherwise the direction gets shifted by the entity's position, which is wrong:
...
// After calculating the reflection, return the direction back to world-space — also without translation.
```
