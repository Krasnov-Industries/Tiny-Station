...
namespace Content.Client._Goob._Shitmed.UserInterface.Systems.PartStatus.Widgets;
...
public sealed partial class PartStatusControl : UIWidget
{
    ...
    public void SetTextures(Dictionary<TargetBodyPart, WoundableSeverity> state)
    {
        foreach (var (bodyPart, integrity) in state)
        {
            ...
            var texture = new SpriteSpecifier.Rsi(new ResPath($"/Textures/_Goob/Interface/Targeting/Status/{enumName.ToLowerInvariant()}.rsi"), $"{enumName.ToLowerInvariant()}_{enumValue}");
